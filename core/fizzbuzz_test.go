...
func TestSameFizzAndBuzzMultiple(t *testing.T) {
	...
	assert.NotNil(t, err, "Error should be thrown when we give the same multiple [%v]", err)
	want := "InvalidParams: The 2 multiples should be different"
	got := err.Error()
...
func TestLimitLowerThanStart(t *testing.T) {
	...
	assert.NotNil(t, err, "Error should be thrown when limit is lower than 1 [%v]", err)

	want := "InvalidParams: Limit should be greater than 1"
	got := err.Error()
	assert.Equal(t, want, got, "Not the good error message [want:%v,got:%v]", want, got)
}
...
func TestSameFizzAndBuzzWord(t *testing.T) {
	...
	assert.NotNil(t, err, "Error should be thrown when we put the same strings [%v]", err)

	want := "InvalidParams: The 2 strings should be different"
	got := err.Error()
	assert.Equal(t, want, got, "Not the good error message [want:%v,got:%v]", want, got)
}
